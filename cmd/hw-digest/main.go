...
func run(ctx context.Context, now time.Time, configPath, seenPath, outputDir string) error {
	...
	for _, set := range cfg.Feeds {
		items, err := collect(ctx, client, set.Sources, now)
		if err != nil {
...
func collect(ctx context.Context, client *http.Client, sources []source, now time.Time) ([]item, error) {
	var result []item
	succeeded := 0
	...
	if succeeded == 0 {
		return nil, errors.New("all source requests failed; existing feed was left unchanged")
	}
	return result, nil
}
...
func parseFeed(r io.Reader, sourceName string) ([]item, error) {
	...
	for {
		...
		date := firstNonEmpty(raw.Date, raw.Updated, raw.Issued, raw.DateDC)
...
func parseTime(value string) (time.Time, error) {
	...
	for _, layout := range []string{time.RFC1123Z, time.RFC1123, time.RFC822Z, time.RFC822, time.RFC3339, "2006-01-02T15:04:05Z", "2006-01-02 15:04:05"} {
...
func writeFeed(dir string, set feedSet, articles []item, now time.Time) error {
	...
	page := fmt.Sprintf("<!doctype html><html lang=\"ja\"><head><meta charset=\"utf-8\"><meta name=\"viewport\" content=\"width=device-width,initial-scale=1\"><title>%s</title><link rel=\"alternate\" type=\"application/rss+xml\" title=\"%s\" href=\"index.xml\"></head><body><h1>%s</h1><p>%s</p><p><a href=\"index.xml\">RSSを購読する</a></p></body></html>\n", escape(set.Title), escape(set.Title), escape(set.Title), escape(set.Description))
